...
use serenity::framework::standard::{
    help_commands, Args, CommandError, CommandGroup, CommandOptions, CommandResult, DispatchError,
...
async fn unknown_command(ctx: &Context, msg: &Message, unknown_command_name: &str) {
    ...
    msg.channel_id.say(ctx, "че) поруски пиши").await.unwrap();
}
...
use serenity::model::guild::Change::Colour;
...
#[command]
async fn niggers(ctx: &Context, msg: &Message) -> CommandResult {
    msg.channel_id.say(&ctx.http, "молодец!!!").await?;
    Ok(())
}
...
async fn owner_check(
    ...
    if msg.author.id != 196295430001131520 {
